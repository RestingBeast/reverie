...
import SummaryCard from "@/components/SummaryCard2";
...
export default function ResultPage() {
    ...
    const handleGenerate = async () => {
      ...
      try {
        ...
        const { spotifyUserId, displayName, avatarUrl, tracks, artists, genres } = await processListeningHistory();
      ...
      <SignOutOverlay isLoggingOut={isLoggingOut} message="Good bye..." />
        ...
        {!summary && !loading && status == "authenticated" && (
          ...
          >
            ...
            <h1 className="text-zinc-400 text-2xl">Hey, {session?.user.name} 👋</h1>
            <p className="text-zinc-400 text-center sm:test-sm">
                While we scan your sonic echoes, your unique narrative is beginning to take shape.
            </p>
            <GenerateButton onClick={handleGenerate} loading={loading} label={"Decode my Sonic Self"} />
          </div>
        )}
